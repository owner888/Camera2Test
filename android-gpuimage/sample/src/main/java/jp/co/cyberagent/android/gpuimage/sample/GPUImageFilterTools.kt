...
object GPUImageFilterTools {
    fun showDialog(
        ...
        val filters = FilterList().apply {
            ...
            addFilter("Highlight Tint Shadow", FilterType.HIGHLIGHT_TINT_SHADOW)
    ...
    private fun createFilterForType(context: Context, type: FilterType): GPUImageFilter {
        return when (type) {
            ...
            FilterType.HIGHLIGHT_TINT_SHADOW -> GPUImageHighlightShadowTintFilter(
                0.0f,
                1.0f,
                1,
                1
    ...
    private enum class FilterType {
        ...
        SATURATION, EXPOSURE, HIGHLIGHT_SHADOW,HIGHLIGHT_TINT_SHADOW, MONOCHROME, OPACITY, RGB, WHITE_BALANCE, VIGNETTE, TONE_CURVE, LUMINANCE, LUMINANCE_THRESHSOLD, BLEND_COLOR_BURN, BLEND_COLOR_DODGE, BLEND_DARKEN,
    ...
    class FilterAdjuster(filter: GPUImageFilter) {
        ...
        init {
            adjuster = when (filter) {
                ...
                is GPUImageHighlightShadowTintFilter -> HighlightShadowTintAdjuster(filter)
        ...
        private inner class HighlightShadowTintAdjuster(filter: GPUImageHighlightShadowTintFilter) :
            Adjuster<GPUImageHighlightShadowTintFilter>(filter) {
            override fun adjust(percentage: Int) {
                Log.d("charco","adjust percentage: $percentage")
                // filter.setShadows(range(percentage, 0.0f, 1.0f))
                // filter.setHighlights(range(percentage, 0.0f, 1.0f))
                filter.setHighlights(percentage*1.0f/100)
            }
        }
